...
- 应用表单新增「下载短链名」字段，填写后该应用的下载短链立即可用，无需发版或改反代配置。
- 应用详情页展示该应用的下载短链（安卓 / Windows / macOS）并支持一键复制。

## [0.2.18] - 2026-07-10

### Fixed

- 修复上传版本弹窗的节奏、占位文案与表单对齐。
- 修复版本详情弹窗的层级与操作区表现。
- 修复 `/applications` 构建测试包入口，并移除详情页重复入口。
- 修复私有路由间重开页面时构建状态可恢复展示。

## [0.2.17] - 2026-07-10

### Fixed

- 测试构建弹窗应用端卡片改为两列布局，显示名称优先使用构建能力接口 app_name，package code 降为辅助信息；构建状态抽屉同步同一显示规则，避免中文名称与 package code 同行挤压。
