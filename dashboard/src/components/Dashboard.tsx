...
import {
  ...
  ChangelogEntry,
  PaginatedResponse,
} from '../hooks/use-query/useAppsQuery';
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const [selectedAppData, setSelectedAppData] =
    React.useState<AppListItem | null>(null);
  const [publishingTuf, setPublishingTuf] = React.useState<
    Record<string, boolean>
  >({});
  const [openArtifactsPopoverId, setOpenArtifactsPopoverId] = React.useState<
    string | null
  >(null);
  const [hoveredArtifactsPopoverId, setHoveredArtifactsPopoverId] =
    React.useState<string | null>(null);
  const [suppressHoverArtifactsPopoverId, setSuppressHoverArtifactsPopoverId] =
    React.useState<string | null>(null);
  ...
  const { data: appData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const app = response.data.apps.find(
        (a: AppListItem) => a.AppName === selectedApp
      );
      return app || null;
    },
    enabled: !!selectedApp,
  ...
  const handleTufPublish = async (e: React.MouseEvent, version: AppVersion) => {
    ...
    try {
      ...
      toastSuccess(
        `TUF artifacts publishing successfully started for version ${version.Version}`
      );
  ...
  const getTufSignStatus = (
    version: AppVersion
  ): 'all-signed' | 'partial' | 'none' | null => {
    if (!appData?.Tuf) {
      return null;
    }
    ...
    const signedCount = tufArtifacts.filter(
      (artifact) => artifact.TufSigned === true
    ).length;
  ...
  const getArtifactSummary = (artifacts: AppVersion['Artifacts']) => {
    const groupedByPlatform = artifacts.reduce<
    ...
    const hiddenGroupsCount = Math.max(
      sortedGroups.length - visibleGroups.length,
      0
    );

    return {
      visibleSummary: visibleGroups
      ...
      <div>
        ...
        <div className='mb-6'>
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 mb-4'>
            ...
            <Select
              value={
                filters.critical === null ? '__all__' : String(filters.critical)
              }
              onValueChange={(v) =>
                setFilters((prev) => ({
                  ...prev,
                    ...
                    <div className='flex items-center justify-end min-w-0 w-full'>
                      <div className='flex gap-2 flex-shrink-0 items-center'>
                        ...
                        <ActionIcons
                          ...
                          showDownload={
                            app.Artifacts.length === 1
                              ? !!app.Artifacts[0].link
                              : true
                          }
                          artifactLink={
                            app.Artifacts.length === 1
                  ...
                  <CardContent className='p-4 pt-2'>
                    ...
                    {app.Artifacts.length > 0 && (
                      <div
                        ...
                        onMouseLeave={() => {
                          setHoveredArtifactsPopoverId((prev) =>
                            prev === app.ID ? null : prev
                          );
                          setSuppressHoverArtifactsPopoverId((prev) =>
                            prev === app.ID ? null : prev
                          );
                        }}>
                        <button
                          ...
                          {artifactSummary.hiddenGroupsCount > 0
                            ? ` +${artifactSummary.hiddenGroupsCount}`
                            : ''}
                        </button>
                        <div
                          className={`absolute left-0 right-0 bottom-full z-20 mb-2 rounded-md border border-border bg-popover p-3 shadow-lg transition-opacity duration-150 max-h-44 overflow-hidden flex flex-col ${
                            ...
                            (hoveredArtifactsPopoverId === app.ID &&
                              suppressHoverArtifactsPopoverId !== app.ID)
                          ...
                          <div className='overflow-y-auto pr-1 flex-1 min-h-0'>
                            {artifactSummary.details.map((detail, index) => (
                              <p
                                key={`${app.ID}-artifact-${index}`}
                                className='text-xs text-muted-foreground break-all mb-1 last:mb-0'>
                                {detail}
                              </p>
                            ))}
                          </div>
                        ...
                        <div className='text-sm text-muted-foreground line-clamp-3 prose prose-sm dark:prose-invert max-w-none'>
                          <ReactMarkdown
                            components={{
                              ...
                              ol: ({ children }) => (
                                <ol className='m-0 pl-4'>{children}</ol>
                              ),
                              li: ({ children }) => (
                              ...
                              h1: ({ children }) => (
                                <h1 className='text-base font-bold m-0'>
                                  {children}
                                </h1>
                              ),
                              h2: ({ children }) => (
