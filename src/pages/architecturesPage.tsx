...
import {
  ...
  Architecture,
} from "../hooks/use-query/useArchitectureQuery";
...
export const ArchitecturesPage = () => {
  ...
  const [deleteConfirmationOpen, setDeleteConfirmationOpen] =
    React.useState(false);
  const [architectureToDelete, setArchitectureToDelete] = React.useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const { architectures, deleteArchitecture, isLoading } =
    useArchitectureQuery();
  const filteredArchitectures = useSearch(
    architectures,
    searchTerm
  ) as Architecture[];
      ...
      {isLoading ? (
        ...
        <div className="text-center text-muted-foreground py-12">
          {searchTerm
            ...
            : "No architectures have been created yet."}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredArchitectures.map((architecture) => (
            <ArchitectureCard
              ...
              onDelete={() =>
                handleDelete(architecture.ID, architecture.ArchID)
              }
            />
          ))}
        </div>
